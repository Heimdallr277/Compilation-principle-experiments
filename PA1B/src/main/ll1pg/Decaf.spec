...
Type            :   AtomType ArrayType
                    {
                        $$ = $1;
                        while(!$2.isLambda.empty()) {
                            if ($2.isLambda.peek()) {
                                $$.type = new TLambda($$.type, $2.typeListStack.peek(), $1.type.pos);
                                $2.typeListStack.pop();
                            } else {
...
SimpleStmt      :   Var Initializer
                ...
                |   VAR Id '=' Expr
                    {
                        $$ = svStmt(new LocalVarDef(Optional.ofNullable(null), $2.id, $3.pos, Optional.ofNullable($4.expr), $2.pos));
                    }
                |   /* empty */
                    {
                        $$ = svStmt(null);
...
Expr            :   FUN '(' VarList ')' ExprLambda
                    {
                        if ($5.block != null) {
                            $$ = svExpr(new Lambda($3.varList, true, $5.block, null, $1.pos));
                        } else if ($5.expr != null) {
                            $$ = svExpr(new Lambda($3.varList, false, null, $5.expr, $1.pos));
                        }
                    }
                |   Expr1
                    {
