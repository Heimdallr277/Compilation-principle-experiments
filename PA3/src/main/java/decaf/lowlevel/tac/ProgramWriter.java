...
public class ProgramWriter {
    ...
    private void buildVTableFor(ClassInfo clazz) {
        ...
        var staticVTable = new VTable(clazz.name+"_static", parent);
        var lambdaVTable = new VTable(clazz.name+"_lambda", parent);
        ...
        for (var method : clazz.staticMethods) {
            if (clazz.isMainClass && method.equals("main")) {
                staticVTable.memberMethods.add(FuncLabel.MAIN_LABEL);
            } else {
                staticVTable.memberMethods.add(ctx.getFuncLabel(clazz.name, method));
            }
        }

        // 5. lambdas
        for (var lambda : clazz.lambdaMethods) {
            lambdaVTable.memberMethods.add(ctx.getFuncLabel(clazz.name, lambda));
        }
        ...
        ctx.putVTable(staticVTable);
        ctx.putOffsets(staticVTable);

        ctx.putVTable(lambdaVTable);
        ctx.putOffsets(lambdaVTable);
