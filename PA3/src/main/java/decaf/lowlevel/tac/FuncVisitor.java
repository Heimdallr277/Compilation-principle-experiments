...
public class FuncVisitor {
    ...
    public static Stack<LambdaSymbol> lambdaBeingDefined = new Stack<>();
    public static Stack<Temp> lambdaTempsBeingDefined = new Stack<>();
    ...
    public Temp visitFuncCall(Temp entry, Temp thisPointer, List<Temp> args, boolean needReturn) {
        Temp temp = null;

        func.add(new TacInstr.Parm(thisPointer));
        for (var arg : args) {
            func.add(new TacInstr.Parm(arg));
        }
        if (needReturn) {
    ...
    public void visitFuncCall(Temp entry, Temp thisPointer, List<Temp> args) {
        visitFuncCall(entry, thisPointer, args, false);
    }

    public Temp visitStaticFuncCall(Temp object, List<Temp> args, boolean needReturn) {
        Temp temp = null;
        var entry = visitLoadFrom(object, 4);
    ...
    public void visitStaticFuncCall(Temp object, List<Temp> args) {
        visitStaticFuncCall(object, args, false);
    }

    public Temp getFuncLabelTemp(Temp thisPointer, String clazz, String method) {
        var vtbl = visitLoadFrom(thisPointer);
        return visitLoadFrom(vtbl, ctx.getOffset(clazz, method));
    }

    public Temp getStaticFuncLabelTemp(String clazz, String method) {
        clazz = clazz + "_static";
        var vtbl = visitLoadVTable(clazz);
        return visitLoadFrom(vtbl, ctx.getOffset(clazz, method));
    }

    public Temp getLambdaLabelTemp(String clazz, String method) {
        clazz = clazz + "_lambda";
        var vtbl = visitLoadVTable(clazz);
        return visitLoadFrom(vtbl, ctx.getOffset(clazz, method));
    }
