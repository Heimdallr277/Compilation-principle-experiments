...
public class ScopeStack {
    ...
    public Optional<Symbol> lookupBefore(String key, Pos pos, ArrayList<String> symbolBeingDefined) {
        Optional<Symbol> ans = findWhile(key, whatever -> true, s -> !(s.domain().isLocalScope() && s.pos.compareTo(pos) >= 0));
        if (ans.isPresent()) {
            if (symbolBeingDefined.contains(ans.get().name)) {
                ans = Optional.empty();
            }
        }
        return ans;
    ...
    public Optional<Symbol> lookupBeforeInLambda(String key, Pos pos) {
        Predicate<Scope> cond = Scope::isLambdaOrLocalScope;
        Predicate<Symbol> validator = (Symbol s) -> !(s.domain().isLocalScope() && s.pos.compareTo(pos) > 0);
    ...
    public Optional<Symbol> lookupOutOfLambdaInFormalScope(String key, Pos pos) {
        Predicate<Scope> cond = Scope::isLambdaOrLocalScope;
        Predicate<Symbol> validator = (Symbol s) -> !(s.pos.compareTo(pos) > 0);
