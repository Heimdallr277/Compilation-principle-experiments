...
public class Typer extends Phase<Tree.TopLevel, Tree.TopLevel> implements TypeLitVisited {
    ...
    private void captureAndCheck(Symbol var, boolean captureThis, ScopeStack ctx) {
        if (!lambdaBeingDefined.empty() && var != null) {
            var stack = new Stack<Tree.Lambda>();
            stack.addAll(lambdaBeingDefined);

            while (!stack.empty()) {
                var outerLambda = stack.pop();
                if (outerLambda.symbol.scope.containsKey(var.name) ||
                        (outerLambda.symbol.scope.nestedLocalScope() != null && outerLambda.symbol.scope.definedInLambda(var.name))) {
                    debug("inspecting: "+var.name+ "@"+var.pos+" in Lambda@"+outerLambda.pos+" ? true");
                    break;
                } else {
                    debug("inspecting: "+var.name+ "@"+var.pos+" in Lambda@"+outerLambda.pos+" ? false");
                    if (captureThis) {
                        outerLambda.symbol.captureThis = true;
                    } else {
                        if (var instanceof VarSymbol) outerLambda.symbol.capture((VarSymbol)var);
                    }
                }
            }
        }
                ...
                if (symbol.get().isVarSymbol()) {
                    ...
                    if (var.isMemberVar()) {
                        if (ctx.currentMethod().isStatic()) {
                            ...
                            expr.setThis();
                            captureAndCheck(var, true, ctx);
                        }
                    } else {
                        captureAndCheck(var, false, ctx);
                    }
                    return;
                }
        ...
        if (symbol.isPresent()) {
            if (symbol.get().isMethodSymbol()) {
                ...
                if (requireStatic) {
                    ...
                    if (expr.getReceiverVar().isPresent()) {
                        captureAndCheck(expr.getReceiverVar().get(), expr.receiverIsThis(), ctx);
                    } else {
                        captureAndCheck(var, expr.receiverIsThis(), ctx);
                    }
...
//            }
            ...
            if (receiver instanceof Tree.Lambda) {
                ...
                expr.symbol = ((Tree.Lambda) receiver).symbol;
        ...
        //debug("Call@"+expr.pos+" is methodSymbol: "+expr.symbol.isMethodSymbol());
    }
...
//        }
    ...
    public void visitLambda(Tree.Lambda lambda, ScopeStack ctx) {
        ...
        ctx.currentClass().lambdas.add(lambda.symbol);


        debug("============ captured symbols @"+lambda.pos+ " =============");
        if (lambda.symbol.captureThis) {
            debug("This ("+lambda.symbol.owner+") @"+lambda.pos);
        }
        for (var symbol : lambda.symbol.capturedVars.entrySet()) {
            debug(symbol.getKey().name+" @"+symbol.getKey().pos);
        }
        debug("============ That's all @"+lambda.pos+ " =============");
    }
